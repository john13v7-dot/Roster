...
(function (global) {
  ...
  var TIME_RE = /^\s*(\d{1,2})\s*[:.hH ]?\s*(\d{2})\s*$/;
  ...
  function coversClosing(hoursText, lateEndMinutes) {
    var parts = String(hoursText).split(/[-–—]/).map(function (p) { return p.trim(); }).filter(Boolean);
    if (parts.length < 2) return false;
    var start, end;
    try {
      start = parseTimeLoose(parts[0]);
      end = parseTimeLoose(parts[parts.length - 1]);
    } catch (e) { return false; }
    if (end <= start) end = ((Math.floor(end / 60) + 12) % 24) * 60 + (end % 60);
    return end >= lateEndMinutes;
  }
        ...
        s.floors.forEach(function (floor) {
          ...
          if (fbAvailable && pair.length && byName[fb].floor === floor && (need.late || 0) > 0) {
            ...
            if (!covered) {
              ...
              // A manual override on the fallback closer's own day is the
              // manager's explicit instruction and wins as-is - only their
              // ordinary, un-overridden hours get extended.
              if (!overrideText(fb, d) && !coversClosing(fbAssignment.text, s.shifts.late.end)) {
                var startParts = fbAssignment.text.split(/[-–—]/).map(function (p) { return p.trim(); }).filter(Boolean);
                var startPart = startParts.length ? startParts[0] : fbAssignment.text;
                cells.set(cellKey(fb, d), { kind: 'static', text: startPart + ' – ' + t12(s.shifts.late.end) });
              }
            }
          }
